...
#keeping high, moderate removing wildtype
removed <- subset(common_patient_mut, 
                  (common_patient_mut$IMPACT == "HIGH" | common_patient_mut$IMPACT == "MODERATE" ) & 
                    common_patient_mut$Variant_Classification != "Silent" & 
                    common_patient_mut$Variant_Classification != "Missense_Mutation" & 
                    !(common_patient_mut$Variant_Type == "INS" & (common_patient_mut$Tumor_Seq_Allele1 == "-" &
                                                                   common_patient_mut$Tumor_Seq_Allele2 == "-")))

common_pat <- substr(removed$Tumor_Sample_Barcode,1,
                                             nchar(removed$Tumor_Sample_Barcode)-3)
common_pat <- unique(common_pat)
```
...
#pca_test <- prcomp(t(mat), scale. = TRUE)
#x <- as.data.frame(pca_test$x)
#reduced.x <- x[1:p,]
#res <- pheatmap(reduced.x,
#                cluster_rows = F,
#                show_colnames=FALSE, 
...
cluster$Tumor_Sample_Barcode <- rownames(cluster)
cluster$Tumor_Sample_Barcode <- substr(cluster$Tumor_Sample_Barcode,1,
                                             nchar(cluster$Tumor_Sample_Barcode)-3)
...
#converts to 0,1
pat_w_pfs$PFS_STATUS <- as.numeric(substr(pat_w_pfs$PFS_STATUS, 1, 1))

#plotting KM plot
#fit, do not change the OS_months or pfs status. just change the "subtype" part to the name of the column with the mutation data that you added

#pat_w_pfs <- subset(pat_w_pfs, pat_w_pfs$clusternum != 3 & pat_w_pfs$clusternum != 4)
fit = survfit(Surv(PFS_MONTHS, PFS_STATUS) ~ clusternum, data=pat_w_pfs)
print(fit)

#plot 
ggsurvplot(fit, data=pat_w_pfs, pval=T, risk.table=T, risk.table.col="strata",
...
###
```{r}
clin_patient_in_cluster <- clin_patient_w_data[which(clin_patient_w_data$PATIENT_ID %in% cluster$Tumor_Sample_Barcode),]

clin_patient_in_cluster$clusternum <- cluster$clusternum
```

```{r}
#Clusters by subtype 
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=SUBTYPE))+
  geom_bar()

#Clusters by age 
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=as.factor(AGE)))+
  geom_bar()

#Clusters by race 
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=RACE))+
  geom_bar()

#clusters by Radiation therapy/not
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=RADIATION_THERAPY))+
  geom_bar()

#clusters by sex 
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=SEX))+
  geom_bar()

#clusters by ethnicity
ggplot(data=clin_patient_in_cluster, aes(x=clusternum, fill=ETHNICITY))+
  geom_bar()

cluster <-  as.data.frame(cutree(res$tree_col, k = 2))
colnames(cluster)[1] <- 'clusternum'

result <- lapply(rownames(cluster), function(x) substr(x, 1, nchar(x) - 3))
rownames(cluster) <- result
```
...
pat_want <- pat_w_pfs$PATIENT_ID

top_10_hugo_rna <- subset(rna_data, select = pat_want)



top_10_hugo_rna <- top_10_hugo_rna[rowSums(top_10_hugo_rna)>100,]


#colData <- subset(cluster, cluster$clusternum != 1)
colData <- cluster
...
p1 <- ggplot(as.data.frame(countData),aes(x=`TCGA-3C-AALI`))+
  geom_histogram(bins=50)+
  labs(title="No transformation")

log2 <- log(countData+1, base = 2)
p2 <- ggplot(as.data.frame(log2),aes(x=`TCGA-3C-AALI`))+
  geom_histogram(bins=50)+
  labs(title="Log(x+1)")
...
dds = DESeq(dds)
#last run at 8:32 am 04/12/23
dds

```
## building results table
```{r}
resdds <- results(dds, contrast = c("clusternum", 1,2))
resdds
#upregulated means upregulated in cluster 1
```
...
resddsSig <- subset(resdds, padj < 0.05)
...
genesdownreg <- order(resddsSig$log2FoldChange, decreasing = FALSE)[1:10]
genesupreg <- order(resddsSig$log2FoldChange, decreasing = TRUE)[1:10]

genes <- c(genesdownreg,genesupreg)
...
library("AnnotationDbi")
library("org.Hs.eg.db")
new_row_names <- sub("\\..*", "", rownames(resdds))
rownames(resdds) <- new_row_names
```

```{r}
resdds$symbol = mapIds(org.Hs.eg.db,
                    keys=row.names(resdds), 
                    column="SYMBOL",
                    keytype="ENSEMBL",
                    multiVals="first")
```
```{r}
resdds$entrez = mapIds(org.Hs.eg.db,
                    keys=row.names(resdds), 
                    column="ENTREZID",
                    keytype="ENSEMBL",
                    multiVals="first")
```
```{r}
resdds$name =   mapIds(org.Hs.eg.db,
                    keys=row.names(resdds), 
                    column="GENENAME",
                    keytype="ENSEMBL",
                    multiVals="first")
...
foldchanges = resdds$log2FoldChange
names(foldchanges) = resdds$entrez
head(foldchanges)
